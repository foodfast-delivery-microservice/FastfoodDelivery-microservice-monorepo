...
function Cart({ cart = [], setCart }) {
  ...
  const getCartKey = useCallback(() => {
    ...
    const identifier = currentUser.id || currentUser.uid || currentUser.phonenumber || currentUser.username;
    if (!identifier) return "cart_guest";
    return `cart_${encodeURIComponent(identifier)}`;
  }, [currentUser]);
  ...
  useEffect(() => {
    ...
    setCart(stored);
  }, [getCartKey, setCart]); // chạy 1 lần khi mount
  ...
  }, [currentUser, getCartKey, setCart]);
